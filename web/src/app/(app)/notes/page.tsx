  ...
  return (
    <div className="mx-auto max-w-lg space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted">Shared scratchpad for the couple.</p>
      </div>
      <label className="block text-sm">
        <span className="font-medium">Pinned</span>
        <textarea rows={2} value={pinned} onChange={(e) => setPinned(e.target.value)} className="mt-1 w-full rounded-lg border border-line px-3 py-2 text-sm" />
        <button type="button" onClick={savePin} className="mt-2 text-xs font-medium underline">
          Save pinned
        </button>
      </label>
      <form onSubmit={add} className="space-y-2">
        <textarea rows={3} value={body} onChange={(e) => setBody(e.target.value)} required placeholder="Add a note…" className="w-full rounded-lg border border-line px-3 py-2 text-sm" />
