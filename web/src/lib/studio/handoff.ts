...
import { designFor, dueDate } from "./design";
export function publicHandoff(p: StudioProject, weddingDate = "") {
  const source = designFor(p);
  return {
    title: p.title,
    qty: p.qty,
    zone: p.zone,
    ...
    materials: p.materials
      .filter((m) => m.qty > 0)
      .map((m) => ({ id: m.id, label: m.label, qty: m.qty, unit: m.unit })),
    tasks: p.steps.map((s) => ({
      id: s.id,
      what: s.what,
