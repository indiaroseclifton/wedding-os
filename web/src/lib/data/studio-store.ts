    ...
    p.id !== id
      ...
      : {
          ...
          materials: p.materials.map((m) =>
            m.id === materialId ? { ...m, bought: !m.bought } : m,
          ),
          updatedAt: new Date().toISOString(),
        },
  );
...
export async function setAfter(
  workspaceId: string,
  id: string,
  afterFate: AfterFate,
) {
...
export async function upsertKindProject(
  ...
  const existing = current.projects.find(
    (p) => !p.design && p.kind === input.kind && p.title === input.title,
  );
...
export async function mutateStudio<T>(
  workspaceId: string,
  change: (projects: StudioProject[]) => T | Promise<T>,
) {
  const { mutateRecord } = await import("@/lib/studio/persistence");
  return mutateRecord<StoredStudio, T>("studio-projects.json", async (all) => {
    const row = all[workspaceId] || {
      workspaceId,
      projects: [],
      updatedAt: new Date().toISOString(),
    };
    const result = await change(row.projects);
